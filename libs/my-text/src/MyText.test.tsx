...
describe('MyText component', () => {
    it('should apply the correct Google Font key based on fontFamily, fontWeight, and fontStyle', () => {
        const { getByText } = render(
            <MyText style={{ fontFamily: 'Nunito', fontWeight: 'bold', fontStyle: 'italic' }}>
                Test Text
            </MyText>
        );
    ...
    it('should preserve the fontVariant small-caps style', () => {
        const { getByText } = render(
            <MyText style={{ fontFamily: 'SourceSansPro', fontVariant: ['small-caps'] }}>
                Small Caps Text
            </MyText>
        );
    ...
    it('should preserve the correct styles across three levels of nesting with SourceSansPro, even with random inner styles', () => {
        const { getByText } = render(
            <MyText style={{ fontFamily: 'SourceSansPro', fontWeight: '600', fontStyle: 'italic', fontVariant: ['small-caps'] }}>
                <MyText style={{ fontStyle: 'normal' }}>
                    <MyText style={{ fontWeight: 'bold', fontVariant: ['common-ligatures'] }}>
                        Nested Text
                    </MyText>
                </MyText>
            </MyText>
    ...
    it('should correctly apply styles to each text segment with nested MyText elements and different font families', () => {
        ...
        expect(foobarbazmew.props.style.fontFamily).toBe('SourceSansPro_600SemiBold_Italic');
