...
const List: React.FC<{ startIndex: number }> = ({ startIndex }) => {
  return (
    <SectionList
      sections={QUIZ_SECTIONS.slice(startIndex).concat(
        QUIZ_SECTIONS.slice(0, startIndex),
      )}
      stickySectionHeadersEnabled
      renderItem={({ item, index, section }) => {
        return (
          <QuizCard data={item} num={index} cardsAmount={section.data.length} />
        );
      }}
      renderSectionHeader={({ section }) => {
        return (
          <TabbedSectionHeader
            tabTestID={section.key ?? ''}
            title={section.key ?? ''}
          />
        );
      }}
